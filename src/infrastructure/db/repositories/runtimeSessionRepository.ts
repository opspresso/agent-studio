...
export const runtimeSessionRepository: RuntimeSessionRepository = {
  async get(sessionId, ownerEmail) {
    const [row] = await sql<StoredRow>(
      "SELECT session_id, owner_email, agent_name, revision, payload, expires_at FROM runtime_sessions WHERE session_id = $1 AND owner_email = $2 AND expires_at > now() AND NOT deleted",
      [sessionId, ownerEmail],
    );
    return row ? {
      sessionId: row.session_id, ownerEmail: row.owner_email, agentName: row.agent_name,
  ...
  async save(row, expectedRevision) {
    const values = [row.sessionId, row.ownerEmail, row.agentName, row.payload, row.expiresAt];
    const rows = expectedRevision === null
      ? await sql<{ revision: string }>(
        `INSERT INTO runtime_sessions (session_id, owner_email, agent_name, payload, expires_at)
         VALUES ($1, $2, $3, $4, $5)
         ...
         WHERE NOT runtime_sessions.deleted AND runtime_sessions.expires_at <= now() AND runtime_sessions.owner_email = EXCLUDED.owner_email
           AND runtime_sessions.agent_name = EXCLUDED.agent_name RETURNING revision`, values,
      )
      : await sql<{ revision: string }>(
        `UPDATE runtime_sessions SET payload = $4, expires_at = $5, revision = revision + 1, updated_at = now()
         WHERE session_id = $1 AND owner_email = $2 AND agent_name = $3 AND revision = $6 AND expires_at > now() AND NOT deleted
  ...
  async delete(sessionId, ownerEmail) {
    ...
    await sql(`INSERT INTO runtime_sessions (session_id, owner_email, agent_name, payload, deleted, expires_at)
      VALUES ($1, $2, '', '', true, $3)
