...
export const slackRunControlRepository: SlackRunControlRepository = {
  async acquire(target) {
    ...
    const key = keys.slackRunLease(target.agentName, target.channel, target.threadTs);
    let acquired = false;
    await transact([
      { kind: "check", key: keys.agent(target.agentName), condition: agentIsLive },
  ...
  async release(target, token) {
    try {
      await deleteItem(keys.slackRunLease(target.agentName, target.channel, target.threadTs), conditions.existsWith("token", token));
    } catch (error) {
  ...
  async requestStop(target, eventTs) {
    ...
    const key = keys.slackRunControl(target.agentName, target.channel, target.threadTs);
    const now = Date.now();
    await transact([
      { kind: "check", key: keys.agent(target.agentName), condition: agentIsLive },
