 ...
 * table's TTL rather than a code path. That is the point — a record whose
...
export const auditRepository: AuditRepository = {
  ...
  async listByDay(day) {
    // Newest first, and fully paginated rather than bounded top-N: the day is
    // already the bound, and an audit answer that silently stopped at 1MB would
    // be worse than no answer — it reads as "that is everything".
