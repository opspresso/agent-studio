...
export const WORKSPACE_TOOL_DEF: ChannelToolDef = { type: "function", function: {
  ...
  description: "Run persistent tasks in a Sandbox. Read options for current_workspace and workdir. The repository allowlist does not create repositories. Before cloning, use check_repository to verify access, an initial commit and the actual base branch. If the user requested a new repository, create and initialize it with the offered repository tool first. A chat keeps one selected Workspace per project: start creates it once; another start returns it WITHOUT running another task. Use run for follow-ups; workspace_id can be omitted when selected. use_workspace selects an existing owned Workspace. attach_repository connects Git to an empty Git-free workdir; it never detaches Git or changes runtime. Set repository AND base_branch for clone/coding; null means Git-free. Use returned workspace_url and approval_url verbatim as browser links; path fields are relative web paths, never hostnames or directories. Use relative paths in workdir. status/wait report actual progress. Use prepare_git for commit, commit-and-push, push to the work branch, pull-request with title/body/draft, merge with pullRequestNumber/headSha from status.pull_request, or push-main for direct fast-forward publication of an already pushed branch. Return approval_url (or the relative approval_path) and STOP for approval, then read status for actual results. close only when the user ends the Workspace; it preserves the selection and files. run and prepare_git restore closed Workspaces. Never close/start to publish, change runtime or recover a tool error. New run cancels pending Git review; executing/uncertain actions stay blocked. NEVER run Git writes, curl GitHub writes, permission/index workarounds or credential requests in native tasks.",
