...
export function routingPromptSummary(task: RoutedModelTask): string {
  const concepts = [
    ...
    ["advanced mathematics", /\b(NP[ -]?(complete|hard)|undecidability|reduction|topology|research[ -]level)\b|NP[ -]?완전|귀착|위상수학|연구 수준/i],
    ["constraint optimization", /\b(schedule|optimal|optimize|dependency|dependencies|minimum|maximum)\b|스케줄|최적|최소|최대|선행|의존|작업자/i],
    ["simple language transformation", /\b(translate|translation|greeting|rewrite)\b|번역|인사|다듬|안녕하세요/i],
...
/** Shared selection and admission for primary SDK turns and focused ModelTask calls. */
export function createCallModelRouter(
  ...
  function inputTokens(task: RoutedModelTask): number {
    return task.inputTokens ?? estimateContextTokens(task.prompt) + task.imageCount * IMAGE_PART_TOKENS;
  }
  async function rejection(model: string, task: RoutedModelTask): Promise<CallRoutingEvent["reason"] | undefined> {
    ...
    if (task.requiresTools && !facts.capabilities.tools) return "capability";
    if (task.requiresStructuredOutput && !facts.capabilities.structuredOutput) return "capability";
    if (task.imageCount && !facts.capabilities.imageInput) return "capability";
    if ((task.purpose === "reasoning" || task.requiresReasoning) && !facts.capabilities.reasoning) return "capability";
    if (task.purpose === "classification" && !facts.capabilities.structuredOutput) return "capability";
    const cap = outputTokens(model, task);
    if (cap < 1) return "budget";
    const context = createRunContextBudget(model, undefined, cap);
  ...
  function estimate(model: string, task: RoutedModelTask): number {
    ...
    const tokens = inputTokens(task) + PROTOCOL_HEADROOM_TOKENS;
  ...
  function reserve(model: string, task: RoutedModelTask) {
    ...
    return { maxOutputTokens, estimatedCostUsd };
  }
          ...
          try {
            ...
            if (decisionModel) {
              ...
              const criteria = Object.fromEntries([...options.keys()].map((key) => [key, {
                fast: "Routine summaries, straightforward classification, translation and greetings. Prefer for simple transformations. Avoid constraint optimization, formal proofs and complex code.",
                general: "General language tasks, explanations, comparisons, code and moderate multi-step work. Small self-contained math, logic, scheduling and constraint problems that a general reasoning model can solve.",
                coding: "Code generation and debugging", reasoning: "Exceptionally difficult multi-step analysis, research-level proofs or large coupled constraints beyond a general model. Do not choose solely because the purpose is reasoning or the task asks for a proof.", vision: "Image understanding",
              }[key]]));
  ...
  return {
    ...
    async select(task: RoutedModelTask, signal?: AbortSignal, continuationModel?: string) {
      ...
      const { maxOutputTokens, estimatedCostUsd } = reserve(selected, task);
      observe({ purpose: task.purpose, model: selected, tier, source, outcome: "selected", attempt: 1,
        estimatedCostUsd, maxOutputTokens, ...(source === "jev" ? decisionDetails : {}) });
      let settled = false;
      return { model: selected, tier, maxOutputTokens, settle(usage: UsageInfo) {
        if (settled) return;
        settled = true;
        state.spentUsd += Math.max(0, usage.costUsd) - estimatedCostUsd;
      } };
    },
    async execute(task: RoutedModelTask, invoke: (model: string) => Promise<RoutedModelResult>, signal?: AbortSignal): Promise<RoutedModelResult> {
      let { selected, tier, source, valid, decisionDetails } = await selection(task, signal);
