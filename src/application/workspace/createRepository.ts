...
export function createWorkspaceRepositoryCreationUseCases(deps: RepositoryCreationDeps) {
  ...
  return {
    async create(projectName: string, input: CreateWorkspaceRepositoryInput, ownerEmail: string): Promise<WorkspaceRepositoryCreationResult> {
      ...
      const existing = await deps.creations.get(projectName, request.repository);
      if (existing) {
      ...
      const current = await policy(projectName);
      ...
      const started: WorkspaceRepositoryCreation = { projectName, repository: request.repository, requestedBy: ownerEmail, fingerprint,
      ...
      if (registered) await recordAudit({ actorEmail: ownerEmail, action: "settings.update", target: auditTarget("workspace-policy", projectName), detail: "Newly created repository automatically registered" }, deps.now());
