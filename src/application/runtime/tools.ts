import { tool, withGenerationSpan, type Tool, type ToolCallOutputContent, type RunContext } from "@openai/agents";
...
export function createRuntimeTools(
  ...
  async function invoke(name: string, args: Record<string, unknown>, display: Record<string, unknown>, callId: string): Promise<CapabilityOutput> {
    ...
    if (name === IMAGE_TOOL_NAME || name === EDIT_IMAGE_TOOL_NAME) {
      ...
      const { usage, ...image } = await withGenerationSpan(async (span) => {
        const result = name === IMAGE_TOOL_NAME
        ...
        span.spanData.model = result.model;
        span.spanData.usage = { input_tokens: result.usage.inputTokens, output_tokens: result.usage.outputTokens, cost_usd: result.usage.costUsd };
        return result;
      });
      // The paid call is complete even if later image bookkeeping fails.
      emit({ usage });
