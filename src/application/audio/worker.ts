...
export interface AudioWorkerDeps {
  ...
  process(agentName: string, id: string, signal: AbortSignal): Promise<unknown>;
  sweep(signal: AbortSignal): Promise<{ deleted: number; failed: number }>;
  refresh(): Promise<unknown>;
}

/** Long jobs run beside polling; one slow source never blocks all other agents. */
    ...
    try {
      ...
      if (room > 0) {
        for (const job of await deps.due(room)) {
          ...
          const task = deps.process(job.agentName, job.id, signal)
