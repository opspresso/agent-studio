...
export function createAudioConfigUseCases(deps: {
  ...
  validate(input: AudioConfigInput, project: string, email: string): Promise<void>;
  now(): Date;
}) {
  return {
    async get(project: string, email: string) {
      await deps.authorize(project, email);
      return deps.configs.get(project);
    },
    async save(project: string, email: string, input: AudioConfigInput, revision: number) {
      await deps.authorize(project, email);
