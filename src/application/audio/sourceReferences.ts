...
export interface SourceReferenceDeps {
  ...
  authorize(agentName: string, userEmail: string): Promise<void>;
...
export function createSourceReferenceUseCases(deps: SourceReferenceDeps) {
  const resolve = async (agentName: string, id: string, userEmail: string) => {
    const reference = await deps.references.get(id);
    if (!reference || reference.agentName !== agentName || reference.userEmail !== userEmail) {
  ...
  return {
    async register(input: { agentName: string; userEmail: string; namespace: string; itemId: string;
      url: string; filename: string; mimeType: string; refresh?: SourceRefresh }) {
      await deps.authorize(input.agentName, input.userEmail);
      ...
      const id = deps.id();
      await deps.references.put({ id, agentName: input.agentName, userEmail: input.userEmail,
        ...
        encryptedUrl: deps.cipher.encrypt(input.url, sourceReferenceContext(input.agentName, id)),
    ...
    async identity(agentName: string, id: string, userEmail: string) {
      await deps.authorize(agentName, userEmail);
      const reference = await resolve(agentName, id, userEmail);
    ...
    importFile: (async (job, context) => {
      await deps.authorize(job.agentName, job.userEmail);
      if (job.source.kind === "file") {
        const agent = audioSourceAgent(job);
        await deps.authorize(agent, job.userEmail);
        const file = await deps.files.metadata(agent, job.source.fileId, job.userEmail);
      ...
      try { metadata = await deps.files.metadata(job.agentName, `${job.id}-source`, job.userEmail); }
      catch (error) {
        if (!(error instanceof NotFoundError)) throw error;
        metadata = job.sourceRefresh ? await refresh() : await resolve(job.agentName, sourceRef, job.userEmail);
      }
      const file = await deps.files.import({ id: `${job.id}-source`, agentName: job.agentName,
        ...
        if (job.sourceRefresh) {
          ...
          const reference = await resolve(job.agentName, sourceRef, job.userEmail);
          url = deps.cipher.decrypt(reference.encryptedUrl, sourceReferenceContext(job.agentName, reference.id));
        }
