import type { SourceReferenceRepository, SourceDownloader } from "@/domain/artifact/sourceReference";
...
export function createSourceReferenceUseCases(deps: SourceReferenceDeps) {
  ...
  return {
    ...
    async identity(projectName: string, id: string, userEmail: string) {
      ...
      return { namespace: reference.namespace, itemId: reference.itemId };
    },
    importFile: (async (job, context) => {
      ...
      try { metadata = await deps.files.metadata(job.projectName, `${job.id}-source`, job.userEmail); }
        ...
        metadata = await resolve(job.projectName, sourceRef, job.userEmail);
      }
      const file = await deps.files.import({ id: `${job.id}-source`, projectName: job.projectName,
        ...
        const url = deps.cipher.decrypt(reference.encryptedUrl, sourceReferenceContext(job.projectName, reference.id));
