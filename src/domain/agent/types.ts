...
export interface SlackIntegration {
   ...
   * in the channels it belongs to, and an agent that names nothing here
...
/**
 * Spend guards for one agent, over two UTC windows — the day and the month.
 * Every threshold is optional and independent: an agent may warn without ever
 * blocking, or block without warning first. Absent means no limit — the shape
 * every agent had before.
 *
 * The day is the grain the usage row is keyed at
 * (`USAGE#{agent} / DATE#{yyyy-MM-dd}`); the month needs no aggregate of its
...
/**
 * Per-agent Telegram bot credentials. Secrets are AES-encrypted at rest.
 *
...
/**
 * Per-agent Microsoft Teams bot credentials — an Azure Bot registration.
...
export interface AgentParameters {
  ...
  /** Opt into agent-managed persistent Workspaces and isolated Sandbox tools. */
  workspaceTools?: boolean;
  /**
   * Whether this Agent's runs may read the Slack workspace its agent's bot
   ...
   * what a run can reach rather than how it answers. Agents are a shared
   * catalog, so anyone who can run this agent can read anything the bot can —
   * which is every channel it was invited to. Inert without a configured,
   * enabled Slack bot on the agent.
   *
