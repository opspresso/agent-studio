 ...
 * same primes the same way. A convention rather than a per-Agent setting,
 * because the setting would only ever name this string.
 *
 * Domain rather than the run site that asks it (`application/execution/
 * memoryRecall.ts`), because the Agent editor reads the same name to say,
...
/**
 * Whether an Agent's bindings could offer `recall` at all — what can be told
 ...
 * leaves `recall` out, so these bindings cannot offer it. `true` is
