...
/**
 * An agent's OAuth Client ID Metadata Document (CIMD).
 *
 ...
 *
 * The agent is **not** looked up. A public endpoint that reads the database
 * per request invites unauthenticated traffic into it, and answering 404 for an
 * unknown name would leak which agents exist to anyone who asks. A document
 * for an agent that does not exist is inert: the authorization it could start
...
export async function GET(_request: Request, ctx: RouteContext): Promise<Response> {
  ...
  // have been an agent here, and would put an arbitrary string in both.
  if (!isSlug(agent)) {
  ...
  return Response.json(clientMetadataDocument(base, agent, (await getServiceBranding()).name), {
    headers: {
