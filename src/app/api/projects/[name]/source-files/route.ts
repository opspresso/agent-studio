...
import { fileRetentionSchema } from "@/app/api/projects/_lib/audioSchemas";
...
export const POST = withMemberAuth(async (user, request: Request, context: { params: Promise<{ name: string }> }) => {
  ...
  try {
    ...
    const file = await runtime.files.import({ id: randomUUID(), projectName: name, userEmail: user.email,
