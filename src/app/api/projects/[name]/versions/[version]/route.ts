...
import { versionRepository } from "@/lib/container";
...
export const PUT = withAuth(async (_user, request: Request, ctx: RouteContext) => {
  ...
  try {
    return Response.json(await updateVersion(versionRepository, name, version, parsed.data));
  } catch (error) {
    return apiError(error);
  }
});
...
export const DELETE = withAuth(async (_user, _request: Request, ctx: RouteContext) => {
  const { name, version } = await ctx.params;
  try {
    await deleteVersion(versionRepository, name, version);
