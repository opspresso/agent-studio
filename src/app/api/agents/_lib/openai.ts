...
import type { RunImage } from "@/application/execution/runAgent";
...
export function toChatCompletion(
  ...
  return {
    ...
    usage: {
      ...
      // depending on which of this agent's two endpoints asked — which is
...
export async function* toChatCompletionChunks(
  ...
  for await (const chunk of source) {
    ...
    if (chunk.image) {
      ...
      // agent delegates drawing), so this is checked before the top-level
