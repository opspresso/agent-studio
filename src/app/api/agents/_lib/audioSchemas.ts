...
export const audioJobSchema = z.object({
  ...
  postprocess: z.object({ agentName: z.string().min(1).max(128).max(128) }).strict().optional(),
  destination: z.object({ serverName: z.string().min(1).max(128), documents: z.boolean(), memories: z.boolean() }).strict().optional(),
}).strict();

export const audioConfigSchema = audioJobSchema.pick({ model: true, language: true, retention: true, postprocess: true, destination: true }).extend({
  model: z.string().min(1).max(256), retention: fileRetentionSchema,
  enabled: z.boolean(), revision: z.number().int().min(0),
  maxActive: z.number().int().min(1).max(MAX_ACTIVE_AUDIO_JOBS),
