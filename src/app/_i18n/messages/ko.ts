...
export const ko: Messages = {
  ...
  "guide.start.body":
    "네 단계이고, 각 단계는 여러분 프로젝트의 탭 하나에서 이뤄집니다. 배포는 버전을 지정하지 않은 호출자가 어느 버전을 받는지를 정하는 일입니다 — 저장한 버전도 이름을 지정하면 이미 실행됩니다.",
  ...
  "guide.words.versionBody":
    "이름이 붙은 프로젝트의 스냅샷입니다 — 모델·프롬프트·도구·턴과 토큰 상한. 버전을 고치면 새로 만드는 것이 아니라 그 자리에서 덮어쓰므로, 호출자가 고정해 둔 버전(배포된 버전 포함)을 고치면 다음 실행부터 그대로 전달됩니다. 배포는 그중 하나를, 번호를 지정하지 않은 요청에 대한 답으로 지정하는 일입니다.",
  "guide.words.run": "Run",
  "guide.words.runBody":
  ...
  "guide.reach.catalogBody":
    "모든 Skill·도구·에이전트를 하나로 묶은 검색 가능한 인덱스입니다. “요청마다 필요한 기능 찾기”를 켜면 실행마다 요청이 실제로 무엇에 관한 것인지로 이 인덱스를 검색합니다 — 그 실행에서 버전의 바인딩을 넓히는 것이지, 대체하지 않습니다.",
  ...
  "guide.reach.memoryBody":
    "이전 실행들이 저장해 둔 것을 첫 토큰 전에 회상합니다 — 버전이 recall 도구를 내주는 메모리 서버를 바인딩했을 때 동작합니다. 없으면 스위치는 아무 일도 하지 않고, 모든 실행이 기억 없이 시작한다고 알려 줍니다.",

  "guide.surfaces.title": "어디서 답하게 할까",
  "guide.surfaces.body":
    "아래 진입점은 모두 같은 엔진으로 돌고, 모든 실행이 같은 비용·트레이스 기록에 남습니다. 호출자가 버전을 지정하지 않으면 배포된 버전을 돌리며, Playground 만 예외로 지금 열어 둔 버전을 돌립니다.",
  ...
  "guide.limits.guards": "한도는 알린 뒤 거절합니다",
  "guide.limits.guardsBody":
    "프로젝트의 일간·월간 한도는 그 프로젝트의 Settings 탭에 있습니다. 알림 임계값을 넘으면 프로젝트가 지정한 창구로 알림이 한 번 가고 실행은 계속되지만, 차단 임계값을 넘으면 창이 넘어갈 때까지 거절합니다 — 일간은 UTC 자정, 월간은 다음 달 1일입니다.",
  ...
  "guide.trouble.slackBody":
    "멘션·다이렉트 메시지, 그리고 지정한 키워드가 맞는 채널 메시지에만 답하고 나머지는 일부러 무시합니다. Integrations 탭의 테스트 버튼이 봇 토큰이 유효한지 확인하고, 봇이 초대된 채널 목록을 보여 줍니다.",
