...
export const ko: Messages = {
  ...
  "workspace.policy.scopeHint": "GitHub 계정 권한과 개별 Git 승인은 별도로 적용됩니다. 모두 비우면 Git 작업이 차단됩니다. 기본값 복원은 현재 규칙을 배포 설정으로 교체합니다. 이미 실행 중인 작업은 별도로 취소해야 합니다.",
  ...
  "guide.workspaces.repositoryBody": "저장소 생성 전에 정확한 owner/name의 Workspace 접근을 확인한다. 관리자는 Project Settings 또는 Settings의 Workspace 저장소 접근에서 저장소와 소유자를 관리하며, 소유자 허용은 앞으로 생성할 저장소도 포함한다. 차단되면 반환된 관리 링크에서 먼저 정책을 변경한다. 허용된 저장소를 생성·초기화한 뒤 Workspace 서버 Git 계정과 기준 브랜치를 검사한다. Git 작업에는 저장소·기준 브랜치를 함께 지정하며 둘 다 선택하지 않으면 Git 없는 공간이다. 나중에 Git을 연결하려면 workdir가 비어 있어야 한다.",
