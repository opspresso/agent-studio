...
import { Alert, Badge, Button, Card, Group, Loader, Stack, TagsInput, Text, TextInput, Title } from "@mantine/core";
    ...
    try {
      const next = await readJson<WorkspacePolicyResponse>(await fetch(endpoint, { method: "PUT", headers: jsonHeaders,
        body: JSON.stringify({ revision: view.revision, rules: reset ? null : {
          ...(repository.trim() ? { repository: repository.trim() } : {}), repositories, repositoryOwners: owners,
        } }) }));
  ...
  return <Card component="section" id="workspace-repositories" style={{ scrollMarginTop: 80 }}>
    <Stack gap="md">
      ...
      {view?.enabled && <>
        ...
        <TagsInput label={t("workspace.policy.owners")} description={t("workspace.policy.ownersHint")} placeholder="owner"
