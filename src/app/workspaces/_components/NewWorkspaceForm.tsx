...
export function NewWorkspaceForm() {
  ...
  const selected = options?.agents.find(option => option.agentName === agent);
    ...
    const load = () => {
      ...
      void fetch("/api/workspaces/options").then(response => readJson<WorkspaceOptionsResponse>(response)).then(data => {
        ...
        if (!optionsInitialized.current) {
          ...
          if (first) { setAgent(first.agentName); setRuntime(first.defaultRuntime); setRepository(null); setCoding(false); }
  ...
  useEffect(() => {
    ...
    if (coding && agent && selected && branchRepository === repository && branchRepository && workspaceAllowsRepository(selected, branchRepository)) void fetch(`/api/workspaces/branches?agent=${encodeURIComponent(agent)}&repository=${encodeURIComponent(branchRepository)}`)
  ...
  }, [coding, agent, repository, branchRepository, selected]);

  async function start() {
    if (!selected || !task.trim() || busy || !selected.runtimes.includes(runtime) || (coding && (!repositoryAllowed || !branch.trim()))) return;
    setBusy(true); setError(null);
    const body = JSON.stringify({ agentName: selected.agentName, runtime, ...(coding ? { baseBranch: branch, repository } : {}),
  ...
  function selectAgent(value: string | null) {
    setAgent(value); setRepository(null); setCoding(false); setBranch("main");
    setRuntime(options?.agents.find(option => option.agentName === value)?.defaultRuntime ?? "command");
  }
  ...
  return <Stack gap="md" maw={640} mx="auto" p={{ base: "sm", sm: "lg" }} h="100%" style={{ overflowY: "auto" }} onKeyDown={onModEnter(() => { void start(); })}>
    ...
    <AgentSuggestion surface="workspace" request={task} candidates={(options?.agents ?? []).map(option => ({ name: option.agentName, displayName: option.displayName }))}
      selected={agent} onSelect={selectAgent} disabled={busy} />
