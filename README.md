...
그렇지 않으면 그림을 조용히 잃는 대신 요청이 거부된다.
...
핸들(`img_1`, `img_2`, …)로 이미지를 지칭하는데, 사용자가 보낸 것과 런이 그린 것을 모두
아우르므로 "이제 밤으로 만들어 줘" 가 어느 쪽에도 통한다 — 그리고 그 핸들은 subagent
transfer 를 타고 넘어가므로, 에이전트가 그림을 전용 image project 에 넘겨 다시 그리는 게
아니라 *편집* 되게 할 수 있다.
...
`Idempotency-Key` 로 24시간 동안 중복을 제거하고, 기본적으로 겹치는 런을 거부한다. 모든
거부 — 비활성, 중복, 사용 중, publish 된 버전 없음 — 는 상태를 가진 history 행이므로,
...
편집하는 방식이 아니라 도구 이름을 하나씩 부르는 Slack 자체의 상태 줄로 나타나며, 새
...
아웃바운드 URL 은 운영자가 제공하고 SSRF 가드를 거치지만, 공개 URL 이면 무엇이든 허용된다
— 신뢰하는 에이전트만 등록하라.
...
복원된다. 스트리밍도 포함이다 — 모델은 실제 값을 결코 보지 못한다. regex 기반이라 그
엔티티들만 다루고, **연결된 MCP 서버가 받는 것은 마스킹하지 않는다**. 이 기능에 기대기 전에
[docs/SECURITY.md](docs/SECURITY.md#pii-필터링-그리고-그것이-멈추는-곳) 를 읽어라.

### 비용, 귀속, 그리고 가드

모든 런은 **누가 그것을 일으켰는지** 를 기록한다 — 사용자, project 토큰(그 소유자로서),
Slack 사용자, webhook, schedule, 또는 인바운드 A2A — trace 에도, caller 별 일일 usage 행에도.
project 카탈로그를 공유한다는 것은 project 이름이 "누가 이걸 썼나" 에 결코 답해 주지 못했다는
뜻이기 때문이다.

각 project 는 **alert** 임계값(한 번 알리고 계속 실행)과 **block** 임계값(윈도가 넘어갈
때까지 모든 런을 `Retry-After` 와 함께 거부)을 두 윈도 — UTC 일과 UTC 월 — 에 걸어 둘 수
있다. 멤버 자신의 tier 도 그의 월 지출에 상한을 건다. 그와 별개로
`MAX_CONCURRENT_RUNS_PER_ACTOR` 가 한 caller 가 동시에 얼마나 많은 런을 진행할 수 있는지를
제한한다. 각 가드가 무엇을 제한하고 무엇을 제한하지 않는지는
