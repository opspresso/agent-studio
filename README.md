...
저장소 접근은 관리자가 Project Settings에서 관리한다. 저장소별 허용 또는 정확한 계정·조직 소유자
허용을 선택하며, 소유자 허용은 새 저장소에도 적용된다. 저장소 생성 전에 정책을 확인하고 실제
clone 전에 서버 Git 계정의 접근·초기 commit을 검사한다.
