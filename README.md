...
저장소 접근은 관리자가 Project Settings에서 고정 목록·소유자 지정·모든 저장소·신규 자동 허용 중
선택한다. 신규 모드는 Agent의 `Workspace.create_repository`가 생성한 저장소를 자동 등록한다.
생성 전에 정책을 확인하고 실제 clone 전에 서버 Git 계정의 접근·초기 commit을 검사한다.
