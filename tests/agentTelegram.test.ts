...
function makeCalls(overrides: Partial<Parameters<typeof updateAgentTelegram>[5]> = {}) {
  const registered: Array<{ token: string; url: string; secretToken: string }> = [];
  const deleted: string[] = [];
  const calls: Parameters<typeof updateAgentTelegram>[5] = {
    getMe,
...
/** `updateAgentTelegram` with the recorded calls and the base URL filled in. */
function update(
  repo: AgentRepository,
  input: Parameters<typeof updateAgentTelegram>[2],
  email: string,
...
describe("updateAgentTelegram", () => {
  ...
  it.each(["replace", "disconnect"])("keeps the existing webhook when %s loses the agent update", async (operation) => {
    const { repo, current } = fakeRepo(makeAgent());
    ...
    await expect(operation === "replace"
      ...
      : disconnectAgentTelegram(repo, "bot-proj", OWNER, secretCipher, calls.deleteWebhook),
...
describe("runtime, binding, test and webhook", () => {
  ...
  it("decrypts credentials only for an enabled bot", async () => {
    ...
    expect(resolveAgentTelegramRuntime(secretCipher, current())).toMatchObject({
      botToken: "42:tok",
      botUsername: "painter_bot",
    });
    await update(repo, { enabled: false }, OWNER);
    expect(resolveAgentTelegramRuntime(secretCipher, current())).toBeNull();
    expect(await resolveTelegramEventBinding(repo, "bot-proj", secretCipher)).toBeNull();
  });

  it("binds an event to the agent's decrypted secret and username", async () => {
    const { repo } = await configured();
    const binding = await resolveTelegramEventBinding(repo, "bot-proj", secretCipher);
    expect(binding).toMatchObject({ agentName: "bot-proj", botToken: "42:tok", botUsername: "painter_bot" });
  ...
  it("refuses credentials moved under another agent name", async () => {
    ...
    expect(() =>
      resolveAgentTelegramRuntime(secretCipher, { ...current(), name: "other" }),
    ).toThrow();
  });
  ...
  it("lists only the current bot's observed destinations for an owner", async () => {
    ...
    await expect(
      listAgentTelegramDestinations(repo, destinations, "bot-proj", OWNER, secretCipher),
    ).resolves.toHaveLength(1);
    expect(list).toHaveBeenCalledWith("bot-proj", 42, 100);
    await expect(
      listAgentTelegramDestinations(repo, destinations, "bot-proj", OTHER, secretCipher),
    ).rejects.toThrow(ForbiddenError);
  });

  it("tests the stored token and reports who the bot is", async () => {
    const { repo } = await configured();
    expect(await testAgentTelegram(repo, "bot-proj", OWNER, secretCipher, getMe)).toEqual({
      ok: true,
      botId: 42,
      botUsername: "painter_bot",
    });
    const bare = fakeRepo(makeAgent());
    expect(await testAgentTelegram(bare.repo, "bot-proj", OWNER, secretCipher, getMe)).toEqual({ ok: false });
  });
  ...
  it("registers the webhook at this deployment's URL with the minted secret", async () => {
    ...
    const result = await registerAgentTelegramWebhook(
      repo,
      "bot-proj",
      OWNER,
      secretCipher,
  ...
  it("disconnects before telling Telegram, keeping credentials dropped when that fails", async () => {
    ...
    const { repo, current } = await configured();
    const deleted: string[] = [];
    await disconnectAgentTelegram(repo, "bot-proj", OWNER, secretCipher, async (token) => {
      expect(current().telegram).toBeUndefined();
      deleted.push(token);
...
describe("revokeAgentTelegramWebhook", () => {
  it("tells Telegram to drop an enabled bot's webhook, and nothing for a disabled or absent one", async () => {
    const { repo, current } = fakeRepo(makeAgent());
    await update(repo, { botToken: "42:tok", enabled: true }, OWNER);
    const deleted: string[] = [];
    await revokeAgentTelegramWebhook(secretCipher, current(), async (token) => {
      deleted.push(token);
    });
    expect(deleted).toEqual(["42:tok"]);
    await update(repo, { enabled: false }, OWNER);
    ...
    await revokeAgentTelegramWebhook(secretCipher, makeAgent(), async (token) => {
      deleted.push(token);
    });
    expect(deleted).toEqual(["42:tok"]);
  });
