...
import type { Agent, AgentConfiguration, AgentParameters } from "@/domain/agent/types";
...
describe("executeAgent image transfer to a subagent", () => {
  ...
  function imageAgentDeps(channel: FakeChannel) {
    ...
    const childConfiguration: AgentConfiguration = {
      ...
      agentName: "simple-image",
      model: "google/gemini-2.5-flash",
      parameters: { piiFiltering: false, imageGeneration: true, imageModel: DEFAULT_IMAGE_MODEL },
    };
    const deps = {
      ...fixture.deps,
      agents: withConfigurations({ get: async (name: string) => (name === "simple-image" ? child : parent) }, ({
        get: async (agent: string) => (agent === "simple-image" ? childConfiguration : null),
  ...
  it("settles the thresholds of an agent it transferred to, not just its own", async () => {
    ...
    const fixture = imageAgentDeps(channel);
    const claims: Array<{ agent: string; kind: string }> = [];
    const guarded = {
      ...fixture.deps,
      agents: {
        get: async (name: string) =>
          name === "simple-image"
            ? { ...(await fixture.deps.agents.get(name))!, costLimits: { alertThresholdUsd: 1 } }
            : fixture.parent,
      },
      usage: {
        ...fixture.deps.usage,
        getDay: async () => ({ costUsd: { "openai/gpt-image-2": 5 } }),
        claimAlert: async (agent: string, _date: string, kind: string) => {
          claims.push({ agent, kind });
          return true;
        },
      },
    } as unknown as ExecutionDeps;
    ...
    expect(claims).toEqual([{ agent: "simple-image", kind: "alert" }]);
  });
  ...
  it("cancels a delegated image tool with its parent run", async () => {
    const controller = new AbortController();
    const fixture = imageAgentDeps(new FakeChannel(transferScript('{"agent_name":"simple-image","message":"draw a fox"}')));
  ...
  it("reports a child that cannot accept the handed-over image without failing the run", async () => {
    ...
    const childConfiguration: AgentConfiguration = {
      ...configurationFixture({ piiFiltering: false }),
      agentName: "text-child",
      // Not in the model registry, so image input is rejected.
      model: "gpt-test",
    };
    const deps = {
      ...fixture.deps,
      agents: withConfigurations({ get: async (name: string) => ({ ...agentFixture(), name }) }, ({ get: async () => childConfiguration, list: async () => [childConfiguration] }).get),
...
describe("executeAgent nested transfer identity", () => {
  ...
  function chainDeps(channel: FakeChannel) {
    ...
    const agents: Record<string, Agent> = {
      "bruce-bot": { ...agentFixture(), name: "bruce-bot" },
      "sample-agent": { ...agentFixture(), name: "sample-agent" },
      "simple-image": {
        ...agentFixture(),
        name: "simple-image",
      },
    ...
    const deps = {
      ...fixture.deps,
      agents: withConfigurations({ get: async (name: string) => agents[name] ?? null }, ({ get: async (agent: string) => configurations[agent] ?? null, list: async () => [] }).get),

      traces: { put: async (trace: Trace) => void traces.push(trace) },
    } as unknown as ExecutionDeps;
    return { deps, traces, top: agents["bruce-bot"] as Agent, configuration: configurations["bruce-bot"] as AgentConfiguration };
  }
  ...
  it("records native nested agents and action tools in one trace hierarchy", async () => {
    ...
    const spans = traces.find((trace) => trace.agentName === "bruce-bot")!.spans;
...
describe("executeAgent registry bindings that no longer resolve", () => {
  ...
  it("does not offer a transfer to an agent that no longer exists", async () => {
    const channel = new FakeChannel([[contentChunk("answered myself"), usageChunk(1, 1)]]);
    const { deps } = executionDepsFixture(channel);
    deps.agents.get = (async (name: string) =>
      name === "alive-agent" ? { ...agentFixture(), name } : null) as ExecutionDeps["agents"]["get"];
...
describe("executeAgent local subagent dispatch", () => {
  ...
  it("pins the clock for the whole run, so a child cannot say a different now", async () => {
    ...
    deps.agents.get = (async (name: string) =>
      name === "summarizer"
        ? { ...agentFixture(), name: "summarizer" }
        : null) as ExecutionDeps["agents"]["get"];
    deps.agents = withConfigurations(deps.agents, async (agentName: string) =>
...
describe("executeAgent hands the conversation to a transferred agent", () => {
  /** A parent whose only subagent is a local agent named `child`. */
  function chainDeps(channel: FakeChannel) {
    const { deps, ...rest } = executionDepsFixture(channel);
    deps.agents.get = (async (name: string) =>
      name === "child" ? { ...agentFixture(), name: "child" } : null) as ExecutionDeps["agents"]["get"];
    deps.agents = withConfigurations(deps.agents, async (agentName: string) =>
      agentName === "child"
        ? { ...configurationFixture({ piiFiltering: false }), agentName: "child" }
        : null) as ExecutionDeps["agents"];
    return { deps, ...rest };
  }
  ...
  it("carries the earlier turns a follow-up refers to", async () => {
    ...
    expect(childTurn).toContain(`${agentFixture().name}: Here is an orange cat.`);
  ...
  it("hands a grandchild the original conversation, not a transcript of a transcript", async () => {
    ...
    deps.agents = withConfigurations(deps.agents, async (agentName: string) =>
      agentName === "child"
        ...
        : agentName === "child2"
          ? { ...configurationFixture({ piiFiltering: false }), agentName: "child2" }
          : null) as ExecutionDeps["agents"];
...
describe("executeAgent subagent recursion guards", () => {
  /** Every agent is an agent that can transfer to `target`, so A -> B -> A is possible. */
  function mutualDeps(channel: FakeChannel, target: (name: string) => string) {
    ...
    deps.agents = withConfigurations(deps.agents, async (agentName: string) => ({
      ...configurationFixture({ piiFiltering: false }),
      agentName,
      subagentList: [{ name: target(agentName) }],
      maxTurn: 50,
    })) as ExecutionDeps["agents"];
    return { deps, recorded };
  }

  it("refuses a transfer back to an agent already on the chain", async () => {
  ...
  it("stops a straight chain at the depth limit", async () => {
    // Each agent transfers to a fresh name, so the cycle guard never fires —
    ...
    deps.agents = withConfigurations(deps.agents, async (agentName: string) => {
      const depth = Number(agentName.replace("a", "")) || 0;
      return {
        ...configurationFixture({ piiFiltering: false }),
        agentName,
...
describe("collectAgentRun non-streaming dispatch", () => {
...
describe("collectAgentRun dispatch carries the caller", () => {
...
describe("a transfer carries who is asking", () => {
  ...
  it("lets the child's own opt-in decide, not its parent's", async () => {
    ...
    // not name the caller is not a statement about the agent it transfers to.
    const channel = script();
    const deps = transferDeps(channel, { piiFiltering: false, callerContext: true });
