...
function imageToolChannel(edit = false): FakeChannel {
  return new FakeChannel([
    [toolCallChunk(0, "image-call", edit ? "EditImage" : "GenerateImage", JSON.stringify({
      prompt: edit ? "make it blue" : "draw a cat", ...(edit ? { image_id: "img_1" } : {}),
    })), usageChunk(1, 1)],
    [usageChunk(1, 1)],
  ]);
}
...
describe("ProjectA2aExecutor", () => {
  ...
  it("publishes an image generated by an Agent tool", async () => {
    const executor = new ProjectA2aExecutor(
      executionDepsFixture(imageToolChannel()),
      projectFixture(),
      imageToolVersion(),
      fakeStore(),
    );
...
describe("ProjectA2aExecutor — what a message may carry", () => {
  it("reports image storage loss on the completed task", async () => {
    const deps = executionDepsFixture(imageToolChannel());
    deps.artifacts = {
  ...
  it("edits the picture sent to an Agent with image tools", async () => {
    const deps = executionDepsFixture(imageToolChannel(true));
    ...
    expect(sources).toEqual([expect.objectContaining({ b64: "AAAA", mimeType: "image/png", id: "img_1" })]);
    expect(statusEvent(bus.events)?.status?.state).toBe(TaskState.TASK_STATE_COMPLETED);
  });
