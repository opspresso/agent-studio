...
export function runtimeSessionFixture(policy: AgentConfiguration["parameters"]["policy"] = {}) {
  ...
  const configuration: AgentConfiguration = { projectName: "project",  model: "google/gemini-2.5-flash", systemPrompt: "Instructions",  parameters: { piiFiltering: true, policy }, skillList: [], mcpList: [], subagentList: [], maxTurn: 5 };
  const scope = { sessionId: "chat-1", ownerEmail: "owner@example.com", projectName: "project", configuration };
  async function run(channel: FakeChannel, message: string, resume?: Parameters<typeof openRuntimeSession>[2], overrides: Partial<AgentDeps> = {}, input: Partial<RunAgentInput> = {}) {
    ...
    for await (const chunk of runAgent({ createToolSchemaValidator, channel, ...overrides }, { projectName: "project", model: configuration.model, parameters: configuration.parameters, maxTurn: 5, messages: [{ role: "user", content: message }], ...input, runtime })) chunks.push(chunk);
    return chunks;
  }
