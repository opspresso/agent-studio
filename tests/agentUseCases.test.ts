...
import type { AgentConfigurationInput } from "@/application/agent/configurationUseCases";
import type { ConfigurationRefRepos } from "@/application/agent/configurationPolicy";
import { putAgentConfiguration, toAgentConfigurationView } from "@/application/agent/configurationUseCases";
...
function configurationFixture(agentName: string, overrides: Partial<AgentConfiguration> = {}): AgentConfiguration {
  return { ...configurationInput(), agentName, ...overrides };
}
function makeAgentRepo(initial: Agent[] = []): AgentRepository {
  let agents = [...initial];
  return {
    get: async name => agents.find(p => p.name === name) ?? null,
    list: async () => agents,
...
const settingsFixture = (initial: AgentConfiguration[] = []): SettingsFixture => new Map(initial.map(c => [c.agentName, c]));
const ALL_REFS_EXIST: ConfigurationRefRepos = {
  ...
  agents: { get: async name => ({ name }) as never },
};
...
async function writeSettings(settings: SettingsFixture, agents: AgentRepository, name: string,
  input: AgentConfigurationInput, email: string, refs = ALL_REFS_EXIST): Promise<AgentConfiguration> {
  const repository: AgentRepository = { ...agents,
    async get(name, options) {
      const agent = await agents.get(name, options);
      return agent ? { ...agent, configuration: settings.get(name) ?? agent.configuration } : null;
    },
    async update(agent, expected) {
      await agents.update(agent, expected);
      if (agent.configuration) settings.set(agent.name, agent.configuration);
    },
  };
  await putAgentConfiguration({ agents: repository, refs, cipher: secretCipher }, name,
    { ...input, expectedUpdatedAt: (await agents.get(name))?.updatedAt ?? "missing" }, email);
  return settings.get(name)!;
}
function settingsView(configuration: AgentConfiguration): AgentConfiguration {
  return toAgentConfigurationView(secretCipher, { ...agentFixture(configuration.agentName), configuration }).configuration!;
}
...
function patchSettings(settings: SettingsFixture, agents: AgentRepository, name: string,
  patch: ConfigurationPatch, email: string, refs = ALL_REFS_EXIST) {
  const existing = settings.get(name)!;
  return writeSettings(settings, agents, name, { ...settingsView(existing), ...patch,
...
describe("model type validation", () => {
  ...
  it("reports a text model without tools as an agent capability mismatch", async () => {
    ...
    ).rejects.toThrow(/does not support tool calling required by agents/);
  });
});
...
describe("MCP binding header overrides", () => {
  ...
  it("preserves endpoint headers when updating only a model and tool selection, and clears them explicitly", async () => {
    const agents = makeAgentRepo([agentFixture("p", { })]);
    const settings = settingsFixture();
    const created = await writeSettings(settings, agents, "p", {
      ...configurationInput(), mcpList: bindingWith({ "X-MCP-Toolsets": "repos,actions" }),
    }, OWNER);
    const updated = await patchSettings(settings, agents, "p", {
    ...
    const cleared = await patchSettings(settings, agents, "p", {
  ...
  it("encrypts override values at rest and never stores plaintext", async () => {
    const created = await writeSettings(
      ...
      makeAgentRepo([agentFixture("p", { })]),
      "p",
      { ...configurationInput(), mcpList: bindingWith({ Authorization: "Bearer agent-secret" }) },
      OWNER,
    );
  ...
  it("refuses a non-owner editing another agent's overrides", async () => {
    const agents = makeAgentRepo([agentFixture("p", { })]);
    const settings = settingsFixture([configurationFixture("p")]);

    await expect(
      patchSettings(settings, agents, "p", {
...
describe("deleteAgent", () => {
  it("rejects a missing agent with NotFoundError (404)", async () => {
    await expect(deleteAgent(makeAgentRepo(), "nope", OWNER)).rejects.toBeInstanceOf(
      NotFoundError,
    );
  });
  ...
  it("rejects deletion by a non-owner with ForbiddenError (403)", async () => {
    await expect(
      deleteAgent(makeAgentRepo([agentFixture("p")]), "p", OTHER),
    ).rejects.toBeInstanceOf(ForbiddenError);
  });
});
...
describe("updateAgent ownership", () => {
  ...
  it("maps a stale agent snapshot to ConflictError", async () => {
    ...
    await expect(
      updateAgent(repo, "p", { displayName: "Renamed" }, OWNER),
    ).rejects.toBeInstanceOf(ConflictError);
  });
});

describe("agentRepository.list paging", () => {
  it("fills a page rather than letting a dropped row end the walk", async () => {
    // `listAgents` stops on a short page, so a page filtered down to fewer
    // rows than were asked for reads as the end of the catalogue — and every
    // agent after it disappears from the console and repair sweep at once.
    store.rows.clear();
    ...
    await expect(agentRepository.list(2)).resolves.toMatchObject([
      { name: "a" },
      { name: "c" },
    ]);
  });
...
describe("agentRepository.delete cascade", () => {
  ...
  it("removes child rows and leaves a name-reserving tombstone", async () => {
    ...
    store.seed([
      row("AGENT#p", "META"),
      row("AGENT#p", "APITOKEN"),
      row("AGENT#p", "WORKSPACEPOLICY"),
      row("USAGE#p", "DATE#2026-01-01"),
      row("USAGE#p", "DATE#2026-01-02"),
      // An unrelated agent must survive the cascade.
      row("AGENT#other", "META"),
    ]);

    await agentRepository.delete("p");

    expect(keysOf()).toEqual([row("AGENT#other", "META"), row("AGENT#p", "META")]);
    expect(await agentRepository.get("p")).toBeNull();
    await expect(agentRepository.create(agentFixture("p"))).rejects.toThrow(
  ...
  it("leaves META marked, and present, when a child delete fails midway", async () => {
    ...
    await expect(agentRepository.delete("p")).rejects.toThrow(/connection reset/);
    expect(keysOf()).toEqual([row("AGENT#p", "APITOKEN"), row("AGENT#p", "META")]);
    const marked = await store.getItem(row("AGENT#p", "META"));
    ...
    await expect(agentRepository.get("p")).resolves.toBeNull();
    await expect(agentRepository.list(100)).resolves.toEqual([]);
  });

  it("lets the owner resume a cascade left marked by a partial failure", async () => {
    store.rows.clear();
    const agent = agentFixture("recover-delete");
    store.seed([
      {
        ...row("AGENT#recover-delete", "META"),
        ...agent,
        entityType: "AGENT",
        GSI1PK: "TYPE#AGENT",
        GSI1SK: agent.name,
      },
      row("AGENT#recover-delete", "APITOKEN"),
    ]);
    vi.spyOn(store, "deletePartition").mockRejectedValueOnce(new Error("connection reset"));

    await expect(
      deleteAgent(agentRepository, agent.name, OWNER),
    ).rejects.toThrow("connection reset");
    await expect(
      deleteAgent(agentRepository, agent.name, OTHER),
    ).rejects.toBeInstanceOf(ForbiddenError);
    await expect(deleteAgent(agentRepository, agent.name, OWNER)).resolves.toBeUndefined();

    expect(keysOf()).toEqual([row("AGENT#recover-delete", "META")]);
    expect(await store.getItem(row("AGENT#recover-delete", "META"))).toMatchObject({
...
describe("model capability validation", () => {
  it("rejects a tools-incapable model on an agent with ValidationError", async () => {
    await expect(
      writeSettings(
        settingsFixture(),
        makeAgentRepo([agentFixture("p", { })]),
