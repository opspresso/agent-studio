...
const model = (
  ...
  type: ModelType = "text",
): ModelConfig & { type: ModelType; wireId: string } => ({
  id,
  provider: id.split("/")[0]!,
  family: id.split("/")[1]!,
  wireId: id.split("/")[1]!,
...
describe("models table", () => {
  ...
  it("toggles the active key and starts a new key ascending", () => {
    expect(nextSort("name", "asc", "name")).toEqual({
      sortKey: "name",
      direction: "desc",
    });
    expect(nextSort("name", "desc", "price")).toEqual({
      sortKey: "price",
      direction: "asc",
    });
  });

  it("ignores a saved provider filter when that provider no longer has any models", () => {
    expect(visible([models[0]!], { provider: "removed-provider" }).map(item => item.id)).toEqual(["openai/z"]);
  });
});
