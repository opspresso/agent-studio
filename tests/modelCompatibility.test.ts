...
const model: ModelConfig = {
  ...
  capabilities: { tools: true, structuredOutput: false, imageInput: false, reasoning: false, decisions: true },
};
...
describe("Agent model compatibility", () => {
  it("rejects a decisions model even if its metadata claims tool support", () => {
    ...
    expect(agentModelRejectReason({ ...model, capabilities: { ...model.capabilities, decisions: false } })).toBeNull();
  });
});
