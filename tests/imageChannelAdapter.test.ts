...
describe("xAI image dialect", () => {
  ...
  it("refuses generated bytes that cannot be read back through artifact access", async () => {
    stubFetch({
      data: [
        {
  ...
  it("refuses an image type the rest of the product cannot render", async () => {
    stubFetch({ data: [{ b64_json: "aW1n", mime_type: "image/avif" }] });

    await expect(
      channel.generateImage({ model: "xai/grok-imagine-image", prompt: "x" }),
    ).rejects.toThrow(/unsupported image type image\/avif/);
  });
