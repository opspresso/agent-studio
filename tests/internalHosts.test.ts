 ...
 * path by which an address someone *typed* can reach a private network, so the
 * boundary is tested from both sides: what it must admit, and everything
 * adjacent that it must not.
 */
...
describe("skipsUrlGuard — declared internal hosts", () => {
  it("admits a host under a declared suffix", () => {
    ...
    expect(skipsUrlGuard({ url: "http://a.b.agent-mcps.svc.cluster.local/mcp" }, CLUSTER)).toBe(true);
  });

  it("admits the suffix itself as an exact host", () => {
    expect(skipsUrlGuard({ url: "http://agent-mcps.svc.cluster.local/mcp" }, CLUSTER)).toBe(true);
  });

  it("accepts a leading dot on the suffix as the same thing", () => {
    const url = "http://mcp-url-fetch.agent-mcps.svc.cluster.local/mcp";
    expect(skipsUrlGuard({ url }, [".agent-mcps.svc.cluster.local"])).toBe(true);
  });
  ...
  it("matches on a label boundary, never a substring", () => {
    ...
    expect(
      skipsUrlGuard({ url: "http://agent-mcps.svc.cluster.local.evil.test/mcp" }, CLUSTER),
    ).toBe(false);
  });
  ...
  it("refuses a single-label suffix", () => {
    ...
    expect(skipsUrlGuard({ url: "http://anything.local/mcp" }, ["local"])).toBe(false);
    expect(skipsUrlGuard({ url: "http://svc.internal/mcp" }, ["internal"])).toBe(false);
  });
  ...
  it("never matches an IP literal", () => {
    ...
    expect(skipsUrlGuard({ url: "http://172.20.255.103/mcp" }, ["20.255.103"])).toBe(false);
    expect(skipsUrlGuard({ url: "http://10.0.0.1/mcp" }, CLUSTER)).toBe(false);
    expect(skipsUrlGuard({ url: "http://[fd00::1]/mcp" }, CLUSTER)).toBe(false);
  });
  ...
  it("only applies to http(s)", () => {
    expect(
      skipsUrlGuard({ url: "file://mcp-url-fetch.agent-mcps.svc.cluster.local/mcp" }, CLUSTER),
    ).toBe(false);
  });

  it("refuses an unparseable url", () => {
    expect(skipsUrlGuard({ url: "not a url" }, CLUSTER)).toBe(false);
    expect(skipsUrlGuard({ url: "" }, CLUSTER)).toBe(false);
  });
