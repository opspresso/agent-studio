...
async function send(spans: TraceSpan[]) {
  const trace: Trace = { traceId: "studio-run", agentName: "agent", status: "completed",
...
describe("native SDK hierarchy in OTLP", () => {
  it("exports child-first completion order under the actual parent without copying sensitive payloads", async () => {
    ...
    expect(spans.get("root-agent")?.parentSpanContext?.spanId).toBe(spans.get("Agent agent")?.spanContext().spanId);
  ...
  it("attaches missing parents to the run and terminates on malformed cyclic metadata", async () => {
    ...
    expect(spans.get("orphan")?.parentSpanContext?.spanId).toBe(spans.get("Agent agent")?.spanContext().spanId);
    expect(spans.get("first")?.parentSpanContext?.spanId).toBe(spans.get("second")?.spanContext().spanId);
    expect(spans.get("second")?.parentSpanContext?.spanId).toBe(spans.get("Agent agent")?.spanContext().spanId);
  });
});
