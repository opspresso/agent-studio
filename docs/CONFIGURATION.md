...
| `ARTIFACT_ACCESS_MODE` | `authenticated` | **runtime** | `authenticated` returns time-limited pre-signed URLs and keeps the bucket private. `public` returns permanent direct S3 URLs; it works only when the bucket policy and S3 Block Public Access settings allow public reads of `artifacts/*` and the legacy `images/*`. Public mode exposes an object to anyone who obtains its URL even though gallery metadata and deletion remain authenticated. An unknown value fails closed to `authenticated`. |
