...
| `RERANKER_BASE_URL` / `RERANKER_MODEL` | 미설정 | `RERANKER_MODEL`은 **models** | 둘을 함께 설정하면 vector 후보를 `/rerank`로 2차 정렬한다. `RERANKER_MODEL`은 배포 기본값이고 `/models`의 Reranker 타입 선택이 DB override한다. 하나만 설정하면 부팅을 거부한다. 미설정이면 기존 cosine/name 순위를 그대로 쓴다. |
...
Text, Image, Embedding, Reranker 모델. 가격, 컨텍스트 윈도, 출력 상한, capability 플래그, 어떤 route 가 그것을
...
파생한다: `embedding: true` 는 Embedding, `reranking: true` 는 Reranker,
`imageGeneration: true` 는 Image, 셋 다 없으면 Text 다. 세 플래그는 동시에 참일 수 없다.
Embedding과 Reranker는 input 가격만 양수이고 `outputPer1M`·`maxTokens` 가
0이다. `/models` 카탈로그에는 네 타입을 모두 표시하지만 version picker와 `/api/models` 는 실행
