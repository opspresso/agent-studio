...
| `S3_BUCKET_NAME` | unset | — | Bucket for what runs produce — generated images and stored documents, under `artifacts/<kind>/`. A row stores the object key, never the URL. The role's grant must cover **`artifacts/*`** (not just the legacy `images/*`) with `s3:PutObject`, `s3:GetObject` and `s3:DeleteObject`. Unset disables persistence entirely: runs still draw, the bytes reach the surface and stop, and the artifacts gallery answers 404. |
| `ARTIFACT_ACCESS_MODE` | `authenticated` | **runtime** | `authenticated` returns time-limited pre-signed URLs and keeps the bucket private. `public` returns permanent direct S3 URLs; it works only when the bucket policy and S3 Block Public Access settings allow public reads of `artifacts/*` and the legacy `images/*`. **A download link is pre-signed in either mode**, because the filename a browser saves under rides on the request signature and S3 refuses `response-*` overrides on an anonymous GET — so in `public` mode a document's address is time-limited while an image's stays permanent. Public mode exposes an object to anyone who obtains its URL even though gallery metadata and deletion remain authenticated. An unknown value fails closed to `authenticated`. |
...
| People one Slack transcript or reaction list resolves to names (at most `5` at once) | `25` | `src/application/slack/workspaceRead.ts` |
