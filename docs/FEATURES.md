...
Agent 단일화와 현재 설정 관리·실행은 구현돼 있다. 요청 기반 모델 역할 선택, 어댑터 개편과
AG-UI 제거는 아직 미구현이며 아래 결정에 따라 진행한다.
...
프롬프트·역량은 Agent의 현재 설정에 저장하고 모든 실행 창구가 이를 사용한다.
기존 데이터는 [Agent 설정 이전](AGENT-MIGRATION.md)으로 보관·변환한다.
...
Agent의 GenerateImage·EditImage 도구로 제공한다. 독립적인 llm·image 프로젝트는 생성하거나 실행하지 않는다.
...
이름 있는 Version·발행·Compare는 제공하지 않는다. 실험을 분리하려면 프로젝트를 복제한다.

### 모델과 프롬프트

현재 기본 모델·fallback 중심 실행을 [요청 기반 모델 역할 선택](#확정된-개편-방향)으로
...
Agent는 대화 메시지를 직접 사용하며 사용자 프롬프트 템플릿을 실행하지 않는다.
...
Chat별 모델 선택, 메시지 수정, 답변 재생성, 대화 공유·내보내기·전문 검색 UI는 없다.
...
모델 숨김은 기존 Agent의 실행 금지와 다르다. 관리자 모델 테스트도 프로젝트의 일반 실행·Usage
