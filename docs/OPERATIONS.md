...
| Background work (`after()`) | — | A Slack event or trigger firing interrupted by an abrupt instance loss is **not resumed** — a run is not idempotent. Trigger rows of both kinds are repaired to `failed` by the sweep — on the scan tick, and on the trigger's own next webhook delivery, so a deployment with no ticker configured still ends up with a correct ledger. A lost Slack event is deliberately not repaired — it leaves no row to finish, only a user without an answer. |
