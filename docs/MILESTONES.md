...
순서는 투입 노력 대비 가치와 운영 위험을 반영한다. M1–M5는 원장·시크릿·외부 통신·
동시성·추적의 정확성을 먼저 확보하고, M6 이후는 비용 통제, 배포 이력, 평가 등
플랫폼 운영 기능을 완성한다. 백로그 항목은 실제 수요가 확인된 후 투자한다.
...
## M4 — 실행 및 데이터 무결성 강화 ✅ 완료

Next.js를 보안 패치 버전으로 갱신하고 production dependency audit를 통과시켰다.
마스킹된 provider API key를 다시 저장할 때 기존 암호문을 재암호화하지 않는다.
MCP, external agent, A2A의 operator-controlled outbound URL은 하나의 경계를 거친다.
모든 redirect hop을 다시 검증하고 cross-origin redirect를 거부하며, A2A Agent Card가
지정한 RPC URL에도 같은 검증을 적용한다.

채팅 메시지는 chat META의 atomic sequence allocator로 번호를 예약하고 conditional
Put으로 중복을 거부한다. 버전 생성은 conditional Put을 사용하며 published version
삭제를 거부한다. Better Auth의 email/token 등 unique lookup은 전용 lock item과
DynamoDB transaction으로 보호한다. 프로젝트/채팅 삭제는 자식 행을 먼저 지우고
미처리 batch가 남으면 실패하며, META 행은 마지막에 삭제한다.

**완료 조건**: 마스킹 provider key 보존, redirect 차단, sequence 예약, version 생성
경합, published version 삭제 거부, partial batch 실패를 단위 테스트로 검증하고
typecheck/test/build/audit가 통과한다.

## M5 — 트레이스 시스템 연결 ✅ 완료

**구현**: `Trace`와 repository port는 `src/domain/trace/`에 있고 DynamoDB adapter는
인프라 계층에 있다. 에이전트 실행은 항상 기록하며 일반/image predict는
`TRACE_SAMPLE_RATE`(기본 0.1)로 샘플링한다.
...
- 저장하는 span 입출력 크기를 제한한다. 긴 메시지 배열은 단순히 잘라내지 않고
  압축한다.
- span에는 원문 prompt/tool 결과 대신 개수·문자 수·token/cost 요약만 저장한다.
