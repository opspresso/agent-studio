...
있으나 application이 infrastructure를 직접 import 하는 곳이 26건 / 14개 파일이다.
특히 `secretEncryption`(7곳)과 `ssrfGuard`(4곳)는 암호화·아웃바운드 정책이라는
정책 관여 협력자인데 포트 없이 하드와이어링돼 있어, 이들을 쓰는 모든 application
테스트가 `AES_ENCRYPTION_KEY` 환경변수와 `vi.mock`에 묶여 있다.
...
- `SecretCipher` 포트를 domain에 정의하고 application 7곳을 주입으로 전환한다.
  필요한 메서드는 실제 사용 표면을 조사해 결정하고, 추측으로 넓히지 않는다.
...
무결과다. application 허용 목록이 26건에서 15건으로 줄고, 목록에서 제거한 11건이
다시 들어오면 테스트가 실패한다. 마스킹 3단계 길이 티어, `enc:v1:` 접두사,
...
수정 없이 통과한다. `vi.mock("@/infrastructure/net/ssrfGuard")` 3건은 fake 주입으로
대체돼 사라진다.
...
- `lib/runtime-settings.ts`를 repository·cipher 주입 형태로 전환한다. 이 모듈을 그대로
  ...
  주입받는 형태로 함께 바꾼다. 프로세스 로컬 30초 TTL 캐시와 `invalidateSettingsCache()`
  동작은 유지한다.
...
`grep -rn "from \"@/" src/shared/`가 모두 무결과다. application 허용 목록이 15건에서
7건으로 줄고, 새 규칙 2개의 허용 목록은 비어 있다. `pnpm build`가 통과해 라우트 핸들러
...
동시에 `runProject.ts`는 789줄에 책임이 여덟 가지이며, 그중 `runRemoteSubagent`는
...
**완료 조건**: application 허용 목록이 비고, `tests/architecture.test.ts`에서 application
허용 목록 배열 자체를 제거해도 테스트가 통과한다. `wc -l src/application/execution/runProject.ts`가
