...
- 모델 검증은 DB의 등록 모델과 프로바이더 연결을 읽는다. 공개 모델 서버를 호출하거나
  내장 카탈로그로 대체하지 않는다. 등록 모델이 유효하지 않거나 연결이 없으면 적용 전에 중단한다.
...
- `image`는 `model`에 도구 호출을 지원하는 텍스트 모델을 반드시 지정한다. 원래 이미지 모델은
  ...
  기존 이미지 모델이 등록되지 않았으면 등록된 `imageModel`도 지정한다. 이전의 고정 size·quality는
