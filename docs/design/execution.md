...
**하나는 주소가 아니라 앱을 통해 나간다.** 다른 모든 읽기는 서명한 오브젝트 URL 이다 — 바이트는
앱을 거치지 않는다. 브라우저에서 실행되는 페이지(`text/html`)만은 그럴 수 없다. 서명된 주소는
sandbox 헤더를 실을 수 없고, 건네진 뒤에는 그것을 연 사람의 권한보다 오래 살며 public 모드에서는
영구다. 그래서 `/api/artifacts/{id}/view` 가 삭제와 같은 술어로 인가한 뒤 바이트로 답하고,
`Content-Security-Policy: sandbox allow-scripts; default-src 'none'` 이 문서를 불투명 오리진에
놓는다. 목록이 한 줄인 것은 표시 가능성이 아니라 그 값어치를 묻기 때문이다 — 브라우저가 알아서
그리는 PDF 는 sandbox 가 필요 없고, 다운로드는 애초에 신뢰를 요구하지 않는다.
