...
**둘은 주소가 아니라 앱을 통해 나간다.** 다른 모든 읽기는 서명한 오브젝트 URL 이다 — 바이트는
...
`inlineViewOf` 가 boolean 이 아니라 종류를 돌려주는 것은 그 둘의 답이 다르기 때문이다.
`text/html` 은 쓰인 그대로 나가고 — 임의의 마크업을 실행하는 것이 sandbox 의 존재 이유다 —
`text/markdown` 은 실행할 것이 없다. 브라우저는 `text/markdown` 을 저장해 버리므로, 채팅
스레드가 쓰는 것과 **같은 렌더러**(`react-markdown` + `remark-gfm`)로 여기서 페이지를 만든다.
같은 리포트가 어느 표면에서 읽히느냐에 따라 다르게 보여서는 안 되고, 저장소에 markdown 방언이
둘이면 표·체크리스트·코드 펜스의 edge case 도 두 벌이 된다. 그 렌더러가 원시 HTML 을 텍스트로
내보내므로 markdown view 는 `allow-scripts` 없이 나간다 — 보장을 이스케이프가 아니라 브라우저가
한다.
