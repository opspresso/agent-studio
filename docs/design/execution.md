...
전달하며 다른 도구를 실행하거나 두 번째 Agent 루프를 만들지 않는다. 도구 안내는 인사·짧은
요약·일상 언어 작업을 직접 처리하고, 비용 또는 추가 능력에 이점이 있는 작업만 위임하도록
한다. 도구 사용 여부는 주 모델이 판단하며 이 안내만으로 비용 절감을 보장하지 않는다.
...
추론 목적에 해당하는 tier를 우선하며 나머지는 일반 tier를 우선한다. 실제 후보 모델이 하나면
