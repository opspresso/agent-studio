...
표면이 전달한 중단 signal은 인터랙티브 deadline과 합쳐 실행에 전달한다. 사용자의 중단과
timeout은 구별하며, 중단 시 이미 받은 텍스트는 보존하고 대기 중인 미디어 전송은 멈춘다.
표면이 제공하는 분산 중단 확인은 모델 호출·결과 전송 경계에서도 실행하며, 전송 단계에서
확인 실패가 발생해도 오류를 알리고 대기 중인 파일을 보내지 않는다.
`finish`는 완료·실패·중단 상태를 받아 미완료 작업을 성공으로 표시하지 않는다.
`stepDone`에는 공통 `isToolErrorText`로 판정한 도구 실패 여부도 전달한다.
