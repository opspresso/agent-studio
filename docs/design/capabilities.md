...
per-skill and file-count caps (the values are in
[CONFIGURATION.md](../CONFIGURATION.md#limits-fixed-in-code)) and excluding symlinks.
`file_path` is normalised and confined to the skill root: no absolute paths, no `..`, no cross-skill access. An overwrite replaces
