...
봇을 DM 하거나 멘션할 수 있다. 그래서 private project 의 런은 시작 전에
`users.info` 의 이메일로 묻는 사람을 식별해 `userMayAccessProject` 에 묻고, 초대되지 않은
사람·이메일을 공유하지 않는 workspace·사람이 보내지 않은 메시지는 한 줄로 거절한다
(`handleSlackEvent`). 거절은 리액션과 상태 줄보다 *앞* 이다 — 접근이 없는 사람에게는 접수
확인조차 하지 않는다. 조회는 갤러리 귀속이 쓰는 것과 같은 캐시된 프로필 읽기이고, 그 주소는
판정에만 쓰이며 프롬프트에는 닿지 않는다 ([SECURITY.md](../SECURITY.md#인가-모델)).
