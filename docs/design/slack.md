...
가지가 없으면 `chat.update`가 상한 이후의 전체 답을 거부하고 flush offset은 마지막 성공에
멈춘다. 마감이 그 뒤를 새 메시지로 보내므로 답이 문장 한가운데에서 끊기고 앞 메시지에는 로딩
표시가 남는다.
...
`delegate_<name>` 행이 hand-off 전체를 대표하며 자식이 돌아올 때 닫힌다. 결과가 싣고 오는
...
**문서 첨부**는 텍스트로 턴에 읽어 들인다([첨부](chat.md#첨부) 참고). 현재 첨부를 먼저 읽고,
남은 문서 개수·문자 예산으로 히스토리의 최신 사람 메시지부터 읽어 원래 턴에 넣는다.
현재와 과거 첨부가 런 하나의 문서 예산을 공유하며, 빠진 과거 문서는 경고로 알린다.
Slack 파일은 `url_private` 뒤에 살고 이 봇의 토큰이
