...
HTML 원본은 수정하지 않는다. `/view`는 HTML 실행·중지·다시 시작 버튼을 제공하며,
원본을 별도 불투명 origin의 sandbox iframe에 넣는다. JavaScript·버튼·입력·canvas·SVG를
보존하므로 단계형 문서의 나머지 화면에도 접근할 수 있다. 원문을 바깥 DOM에 삽입하지 않는다.
일반 웹 요청과 외부 이동을 제한하지만 완전한 무통신 sandbox는 아니며, 신뢰하는 파일만
명시적으로 실행한다. 세부 경계는 [보안](../SECURITY.md#데이터-노출과-보존)을 따른다.

SaveFile 안내는 외부 CDN·fetch·storage에 의존하지 않는 단일 HTML을 요구하고, 저장이
브라우저 실행 검증을 뜻하지 않는다고 명시한다. 파일 내부의 로직 오류까지 자동으로 고치지는 않는다. 실행 중 감지된 script 오류는 고정된 안내로 표시한다.
`pnpm test:html-preview`는 제어 동작과 주요 보안 경계를 실제 Chromium에서 검증한다.

![HTML 실행 전](images/html-preview-ready.png)
...
프로젝트 실행 API, AG-UI, Slack·Telegram·Teams도 같은 첨부 보관 유스케이스를 사용한다.
각 실행에 제공한 원본은 해당 actor와 project에 귀속되며, 이력에서 다시 첨부하는 바이트는
그 실행의 첨부 사본으로 보관한다. `File` 도구는 사용자에게 보인 문서의 ID를 받아 작업한다.

생성 파일의 HTTP·OpenAI·AG-UI 응답은 다운로드 URL과 `fileId`를 함께 제공한다.
A2A는 파일 URL part의 `metadata.fileId`에 같은 값을 넣는다. URL이 만료돼도 원본이
