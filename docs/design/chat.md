...
바로 그 비용이다. **`attach` 로 이어받은 런에는 숫자가 없다**: 새로고침 뒤에 붙은 런은 이미
얼마간 돌고 있었고 그것이 얼마인지는 wire 어디에도 없으므로, 0부터 세는 시계는 1분 된 답변을
방금 시작한 것으로 보고하게 된다.
...
답변이 같은 것을 말하는 타임스탬프 두 개를 이미 나르고 있다. 짝짓기는 `seq` 를 따라 걸어
