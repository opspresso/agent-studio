...
시계는 **보내기를 누른 순간이 아니라 head frame 에서** 시작한다. 턴의 첨부는 요청 본문으로
올라가므로, 느린 업링크의 4MB 이미지는 누름과 서버가 턴을 받아들인 시각 사이에 수 초를 만든다 —
그리고 그 자리를 이어받는 배지는 서버의 시각에서 재므로, 누름부터 세면 두 숫자가 업로드 길이만큼
어긋난 채 같은 자리에 같은 사실로 놓인다. 같은 이유로 **`attach` 로 이어받은 런에는 숫자가
없다**: 새로고침 뒤에 붙은 런은 이미 얼마간 돌고 있었고 그것이 얼마인지는 wire 어디에도 없으므로,
0부터 세는 시계는 1분 된 답변을 방금 시작한 것으로 보고하게 된다. 재접속의 head frame 도 시계를
다시 시작하지 않는다 — 처음 배운 id 하나만 시작이다.

스트림이 끝나면 그 자리는 **멈춘 소요 시간**이 이어받는다(`endedAtMs`). 저장된 메시지가 자기
배지를 달고 오지만 그것은 retire 의 fetch 가 돌아온 뒤이고, 그 fetch 가 끝내 실패하면 그 턴은
라이브 항목에서 그려진 채로 남는다 — 길이를 가장 알고 싶은 런이 그 길이를 말하지 못하는 경우다.
