...
Agent Studio 는 기업이 **자기 네트워크 안에 설치해 운영하는** 플랫폼이다. 외부 네트워크가
없는 IDC 에서도 부팅·로그인·런이 되어야 하고, 밖으로 나가는 연결은 전부 선택이다. 이 문서는
그 설치의 전 과정이다 — 무엇이 필요한지, 어떻게 올리는지, 폐쇄망에서는 무엇을 대신 하는지,
그리고 기존 AWS 배포에서 어떻게 옮겨 오는지. 설정값 하나하나의 뜻은
...
| 컨테이너 런타임 | 앱 이미지 `ghcr.io/opspresso/agent-studio` | Docker Compose(호스트 하나) 또는 Kubernetes(Helm 차트) |
...
서버들 + 티커. 호스트 한 대(4 vCPU / 8GB 권장, 4GB 는 동작하지만 빌드는 못 한다)에:
...
`deploy.sh` 는 `.env.example` 에서 `.env` 를 생성하고(`.env` 는 손대지 않는다 — 다음 실행이 덮어쓴다),
첫 실행에 **`.env.host`** 를 만든다: 이 호스트의 Postgres·MinIO 자격 증명이고, 다시 생성되지 않는다.
...
백업은 `scripts/backup.sh [DEST]` — `pg_dump` + 오브젝트 미러 + `.env.host`. 복원 절차는 스크립트
머리에 있다.
...
| 스킬·MCP 레지스트리(agent-plugins) | GitHub API 로 sync | `/plugins` 에서 체크아웃의 `.tar.gz` 를 업로드한다(`POST /api/plugins/sync/upload`) — 같은 sync, 입력만 다르다. GitHub Enterprise 는 `GITHUB_API_URL` |
...
- Compose: `.env.example` 의 태그를 올리고(또는 GitHub 이 닿는 호스트는 `deploy.sh` 가 argocd-env-demo 의
  alpha 핀을 따른다) `scripts/deploy.sh`. 스키마 마이그레이션은 앱이 부팅 때 advisory lock 아래에서
  적용한다 — 인스턴스가 여럿이어도 한 번만.
