...
Agent 설정의 호출 단위 자동 라우팅도 같은 결정 모델을 사용한다. 내부 text 모델을 tier에
배정하고 self-hosted 연결 제한을 설정하면 사내 구성으로 사용할 수 있다. 공개 Jev 연결을
사용할 때 전달되는 정보는 작업 목적·제한된 요약·기능·예산·tier다. 결정 모델이 없거나
오프라인이면 주 모델로 fallback하며 필수 실행 경로는 공개 인터넷에 의존하지 않는다.
